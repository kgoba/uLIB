...
template <long baudrate, bool use2x = false> class SimpleSerial {
public:
  ...
  /// Enable serial port.
  static void enable() { UCSR0B = bit_mask2(RXEN0, TXEN0); }

  /// Disable serial port.
  static void disable() { UCSR0B = 0; }

  static bool readByte(byte &b, bool noCheck = false) {
    if (!noCheck && !isRXComplete())
      return false;
    b = UDR0;
    return true;
  }
  ...
  static bool writeByte(byte b, bool noCheck = false) {
    if (!noCheck && !isTXReady())
      return false;
    UDR0 = b;
    return true;
  }

  static bool isRXComplete() { return bit_check(UCSR0A, RXC0); }

  static bool isTXReady() { return bit_check(UCSR0A, UDRE0); }

  static bool isTXComplete() { return bit_check(UCSR0A, TXC0); }
...
template <class Serial, word rxTimeout = WaitableBase::WAIT_FOREVER,
          word txTimeout = WaitableBase::WAIT_FOREVER>
...
public:
  ...
  static void disable() { Serial::disable(); }
  ...
  static byte availableRX() { return rxQueue.count(); }
...
Queue<byte, rxBufferSize>
    BufferedSerial<Serial, rxBufferSize, txBufferSize>::rxQueue;

template <class Serial, int rxBufferSize, int txBufferSize>
Queue<byte, txBufferSize>
    BufferedSerial<Serial, rxBufferSize, txBufferSize>::txQueue;
...
public:
  ...
  static bool isRXComplete() { return rxComplete; }
